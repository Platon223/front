...
async function checkStatus() {
    if(localStorage.getItem('loginName') === null) {
        ...
        document.querySelector('.greeting').innerHTML = `Hi, ${user.name}!`;
...
async function searchPr() {
    ...
    if(inputPr.includes('rod') || inputPr.includes('Rod')) {
       window.location.href = "https://store-7.onrender.com/search/rods";
...
async function fetchProducts() {
    ...
    products.forEach(pr => {
        ...
        prElement.innerHTML = `<div class="product-card">
        <img
